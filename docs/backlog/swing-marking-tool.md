...
- **Is Mode 2 a paid-tier differentiator?** — STILL OPEN. `entitlementActive` is threaded through `shouldShowMarking` and echoed in the decision, but deliberately not enforced; the caller applies policy once you decide. (`shouldShowMarking` takes `entitlementActive` and echoes it in the decision, but does **not** gate on it — the caller applies whatever policy this question lands on.)
- ~~Do we re-extract and re-mark historical swings?~~ **Architecturally yes** — re-extraction from retained video is the chosen design (`docs/memory-architecture-decision-2026-08-02.md`). Open question is only *when* to trigger it: lazily on first comparison, or as a batch backfill. (Today: forward only. Un-marked history is invisible to Mode 2 — a then-vs-now comparison needs a marking on **both** sides, so side-by-side stays unavailable until a user has two marked sessions.)
