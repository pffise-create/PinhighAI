...
function sanitizeVisualObservations(aiAnalysis) {
  const observations = Array.isArray(aiAnalysis?.visual_observations)
  ...
  return observations
    ...
    .map((obs, index) => ({
      ...
      observation: obs.observation.slice(0, 400),
      confidence: typeof obs.confidence === 'number' && Number.isFinite(obs.confidence)
        ? obs.confidence
        : null,
      impact: typeof obs.impact === 'string' ? obs.impact : 'neutral',
      evidence_markers: Array.isArray(obs.evidence?.markers) ? obs.evidence.markers.slice(0, 5) : [],
    }));
}
...
function buildProfileItem({ userId, analysisId, aiAnalysis, analysisResults, existingProfile, timestamp }) {
  ...
  return {
    ...
    visual_observations: sanitizeVisualObservations(aiAnalysis),
