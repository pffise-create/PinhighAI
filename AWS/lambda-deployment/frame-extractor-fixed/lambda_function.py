...
def extract_motion_timeline(video_path, video_info):
    ...
    duration = video_info['duration']
    sample_interval = 0.2  # Sample every 0.2 seconds
...
def extract_p1_p10_frames(video_path, swing_data):
    """Extract frames every 0.25 seconds from the detected swing"""
    ...
    current_time = swing_start
    frame_count = 0
    
    while current_time <= (swing_start + swing_duration):
        # Extract frame
        frame_path = f'/tmp/frame_{frame_count:03d}_{int(current_time*1000)}.jpg'
        ...
        if os.path.exists(frame_path):
            extracted_frames.append({
                'phase': f'frame_{frame_count:03d}',
                'timestamp': current_time,
                'frame_path': frame_path,
                'swing_percentage': (current_time - swing_start) / swing_duration
            })
            
            print(f"Extracted frame_{frame_count:03d} at {current_time:.2f}s")
        
        current_time += frame_interval
        frame_count += 1
        
        # Safety limit - max 20 frames (5 seconds / 0.25s = 20)
        if frame_count >= 20:
            break
...
def upload_frames_and_create_analysis(extracted_frames, bucket_name, analysis_id, swing_data):
    ...
    try:
        ...
        for i, frame_data in enumerate(extracted_frames):
            ...
            # Resize for OpenAI compatibility (target max 15MB base64, ~11MB JPEG)
            image = Image.open(frame_data['frame_path'])
            
            # Target dimensions that should keep file size reasonable
            # Start with larger size and reduce if needed
            max_dimension = 1920  # 1920x1080 is good quality
            ...
            max_size_mb = 10  # Target 10MB max (safe margin under 15MB)
            file_size_mb = len(img_bytes) / (1024 * 1024)
            
            if file_size_mb > max_size_mb:
                print(f"🔄 Frame {frame_data['phase']} is {file_size_mb:.1f}MB, reducing quality...")
                ...
                if file_size_mb > max_size_mb:
                    print(f"   Still too large, reducing dimensions...")
                    for max_dim in [1280, 1024, 800]:
                        image_copy = Image.open(frame_data['frame_path'])
                        image_copy.thumbnail((max_dim, max_dim), Image.Resampling.LANCZOS)
                        
                        img_byte_array = BytesIO()
                        image_copy.save(img_byte_array, format='JPEG', quality=75, optimize=True)
                        img_bytes = img_byte_array.getvalue()
                        file_size_mb = len(img_bytes) / (1024 * 1024)
                        
                        print(f"   Dimension {max_dim}: {file_size_mb:.1f}MB")
                        if file_size_mb <= max_size_mb:
                            image = image_copy
                            break
            
            print(f"✅ Final frame {frame_data['phase']}: {image.size}, {file_size_mb:.1f}MB")
...
def trigger_ai_analysis(analysis_id, user_id):
    ...
    try:
        ...
        # Don't fail the entire process if AI trigger fails
        print("Frame extraction completed successfully despite AI trigger failure")
