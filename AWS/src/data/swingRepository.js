...
function parseAnalysisResults(rawValue, context) {
  if (!rawValue) {
    return null;
  }
  ...
  try {
    ...
    const analysisId = context?.analysisId || 'unknown-analysis';
    // Degrade gracefully for legacy malformed records so chat/history flows still work.
    console.warn(`Failed to parse analysis_results for ${analysisId}: ${error.message}`);
    return null;
  }
}
...
function normalizeAnalysisItem(item) {
  ...
  const analysisResults = parseAnalysisResults(item.analysis_results, { analysisId: item.analysis_id });
  ...
  return {
    ...
    visualObservations: Array.isArray(aiAnalysis?.visual_observations) ? aiAnalysis.visual_observations : [],
  };
}
...
function buildAnalysisReport(item) {
  ...
  return {
    ...
    visualObservations: normalized.visualObservations,
  };
}
